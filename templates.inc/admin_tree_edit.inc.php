...
<div class="main">
	...
	<div class="add-table">
		<div class="table-header">
			<div class="cell-1" style="width: 100%;"><?php echo (voc('Edit tree')); ?> - <?php echo (getVFValue('d_name', LANG)); ?></div>
		</div>
		<div class="add-item">
			<form id="pform">
					...
					<?php foreach($parser["langs"] as $_key_1=>$_var_1)
{
 ?>
					<div class="half-block" style="width: 33%">
						...
						<input type="text" class="prev" id="name_<?php echo $parser["langs"][$_key_1]["ID"]; ?>" name="name_<?php echo $parser["langs"][$_key_1]["ID"]; ?>" value="<?php echo (htmlspecialchars(getVFValue('d_name', ''.$parser["langs"][$_key_1]["ID"].''))); ?>" />
					</div>         
					<?php 
}
 ?>
					<?php foreach($parser["langs"] as $_key_2=>$_var_2)
{
 ?>
					<div class="half-block" style="width: 33%">
						...
						<input type="text" class="prev" id="url_<?php echo $parser["langs"][$_key_2]["ID"]; ?>" name="url_<?php echo $parser["langs"][$_key_2]["ID"]; ?>" value="<?php echo (htmlspecialchars(getVFValue('d_url', ''.$parser["langs"][$_key_2]["ID"].''))); ?>" />
					</div>         
					<?php 
}
 ?>
...
{
 ?>
							<option value="<?php echo $parser["tr"][$_key_3]["ID"]; ?>" <?php if ($parser['d_SUB_ID']==''.$parser["tr"][$_key_3]["ID"].'') {?> 	selected <?php } ?> ><?php echo $parser["tr"][$_key_3]["NAME"]; ?></option>
							<?php 
}
 ?>
						</select>
					...
					<div class="half-block" style="width: 33%">
						...
						<select name="type">
							<option value="1" <?php if ($parser['d_TYPE']=='1') {?> 	selected <?php } ?> ><?php echo (voc('text')); ?></option>
							<option value="2" <?php if ($parser['d_TYPE']=='2') {?> 	selected <?php } ?> ><?php echo (voc('catalogue')); ?></option>
							<option value="3" <?php if ($parser['d_TYPE']=='3') {?> 	selected <?php } ?> ><?php echo (voc('gallery')); ?></option>
							<option value="4" <?php if ($parser['d_TYPE']=='4') {?> 	selected <?php } ?> ><?php echo (voc('contacts')); ?></option>
							<option value="5" <?php if ($parser['d_TYPE']=='5') {?> 	selected <?php } ?> ><?php echo (voc('map')); ?></option>
							<option value="7" <?php if ($parser['d_TYPE']=='7') {?> 	selected <?php } ?> ><?php echo (voc('popup')); ?></option>
						</select>
					</div>
					...
					<?php foreach($parser["langs"] as $_key_4=>$_var_4)
{
 ?>
					<div class="half-block" style="width: 33%">
						...
						<input type="text" class="prev" id="title_<?php echo $parser["langs"][$_key_4]["ID"]; ?>" name="title_<?php echo $parser["langs"][$_key_4]["ID"]; ?>" value="<?php echo (htmlspecialchars(getVFValue('d_title', ''.$parser["langs"][$_key_4]["ID"].''))); ?>" />
					</div>         
					<?php 
}
 ?>
					<?php foreach($parser["langs"] as $_key_5=>$_var_5)
{
 ?>
					<div class="half-block" style="width: 33%">
						...
						<input type="text" class="prev" id="kw_<?php echo $parser["langs"][$_key_5]["ID"]; ?>" name="kw_<?php echo $parser["langs"][$_key_5]["ID"]; ?>" value="<?php echo (htmlspecialchars(getVFValue('d_kw', ''.$parser["langs"][$_key_5]["ID"].''))); ?>" />
					</div>         
					<?php 
}
 ?>
					<?php foreach($parser["langs"] as $_key_6=>$_var_6)
{
 ?>
					<div class="half-block" style="width: 33%">
						...
						<input type="text" class="prev" id="desc_<?php echo $parser["langs"][$_key_6]["ID"]; ?>" name="desc_<?php echo $parser["langs"][$_key_6]["ID"]; ?>" value="<?php echo (htmlspecialchars(getVFValue('d_desc', ''.$parser["langs"][$_key_6]["ID"].''))); ?>" />
					</div>         
					<?php 
}
 ?>	
				...
				<div class="mtexts" id="mtexts">
					...
					<?php foreach($parser["blocks"] as $_key_7=>$_var_7)
{
 ?>
					<?php if (''.$parser["blocks"][$_key_7]["TYPE"].''=='1') {?> 	
					<?php echo (getTextBlock('tree', ID, ''.$parser["blocks"][$_key_7]["ID"].'', ''.$parser["blocks"][$_key_7]["LEFT"].'')); ?>
					 <?php } ?> 
					<?php if (''.$parser["blocks"][$_key_7]["TYPE"].''=='2') {?> 	
					<?php echo (getImageBlock('tree', ID, ''.$parser["blocks"][$_key_7]["ID"].'', ''.$parser["blocks"][$_key_7]["LEFT"].'', '1')); ?>
					 <?php } ?> 
					<?php 
}
 ?>
				...
				<div class="add-item-line">
					<div class="add-item-img-container" id="itemimg">
						<?php foreach($parser["images"] as $_key_8=>$_var_8)
{
 ?>	
						<p class="itemimg itembimg" data-type="<?php echo (getImageType(''.$parser["images"][$_key_8]["TYPE"].'')); ?>" id="up<?php echo $parser["images"][$_key_8]["ID"]; ?>" data-img="<?php echo $parser["images"][$_key_8]["ID"]; ?>"><a style="display: block;" class="del" href="javascript:delImage('<?php echo $parser["images"][$_key_8]["ID"]; ?>', '');"></a><input type="hidden" name="itemimg[]" value="<?php echo $parser["images"][$_key_8]["ID"]; ?>" id="itemimg<?php echo $parser["images"][$_key_8]["ID"]; ?>" /><img src="/getimage.php?id=1<?php echo (getImageType(''.$parser["images"][$_key_8]["TYPE"].'')); ?>1<?php echo $parser["images"][$_key_8]["ID"]; ?>" /><a class="upproc" style="display: none; margin-left: 11px; margin-top: 20px;">0%</a></p>
						<?php 
}
 ?>
...
{
 ?>
		tt = tt + '<div class="full-block" style="width: 33%;"><h1 style="padding: 10px;"><?php echo (voc('Title')); ?> (<?php echo $parser["langs"][$_key_9]["SHORTNAME"]; ?>)</h1><input  style="text-transform: none;" name="bname['+tnm+'][<?php echo $parser["langs"][$_key_9]["ID"]; ?>]" /></div>';
		<?php 
}
 ?>
...
{
 ?>
		t = t + '<div class="full-block" style="width: 33%;"><h1 style="padding: 10px;"><?php echo (voc('Text')); ?> (<?php echo $parser["langs"][$_key_10]["SHORTNAME"]; ?>)</h1><textarea class="prev"  style="height: 160px; text-transform: none;" name="text['+tnm+'][<?php echo $parser["langs"][$_key_10]["ID"]; ?>]" id="text_'+tnm+'_<?php echo $parser["langs"][$_key_10]["ID"]; ?>"></textarea></div>';
		<?php 
}
 ?>
		t = t + '</div></div>';
		...
		<?php foreach($parser["langs"] as $_key_11=>$_var_11)
{
 ?>
		CKEDITOR.replace( 'text_'+tnm+'_<?php echo $parser["langs"][$_key_11]["ID"]; ?>' , {
			toolbar: [ 
...
function saveText()
{
	...
	$.ajax({
		type: 'POST',
		url: '/<?php echo (cLANG); ?>/manager/tree/editajax/<?php echo (ID); ?>/',
...
DJS.addUpImages2 =  function(up, file) {
	var h = '<p class="itemimg itembimg" data-type="" id="upi'+file.id+'" data-img=""><a class="del" href="javascript:delIcons(\''+file.id+'\', \'\');"></a><input type="hidden" name="itemicons[]" value="" id="itemicon'+file.id+'" /><img src="" style="display:none;" /><a class="upproc" style="margin-left: 11px; margin-top: 20px;">0%</a></p>';
	$('#addpbut2').before(h);
	$( "#itemicon" ).sortable({
		containment: "parent",
		items: "p.itemimg",
...
DJS.uploadUpImages2 =  function(up, file, resp) {
	...
	$('#upi'+file.id+' img').attr('src', '/loads/treeicons/'+obj.id+'.svg');
...
$(document).ready(function() {
	...
	<?php foreach($parser["langs"] as $_key_12=>$_var_12)
{
 ?>
	/*
	CKEDITOR.replace( 'stext_<?php echo $parser["langs"][$_key_12]["ID"]; ?>' , {
		toolbar: [ 
			{ name: 'document', items: [ 'Source', '-', 'NewPage' ] }, [ 'Cut', 'Copy', 'Paste', 'PasteText', 'PasteFromWord', '-', 'Undo', 'Redo' ], { name: 'basicstyles', items: [ 'Bold', 'Italic', 'Underline' ] },
			{ name: 'links', items: [ 'Link', 'Unlink', 'Anchor' ] },
			{ name: 'insert', items: [ 'Image', 'Flash', 'Table', 'HorizontalRule', 'Smiley', 'SpecialChar', 'PageBreak', 'Iframe' ] },
	...
	CKEDITOR.replace( 'text_<?php echo $parser["langs"][$_key_12]["ID"]; ?>' , {
		toolbar: [ 
			{ name: 'document', items: [ 'Source', '-', 'NewPage' ] }, [ 'Cut', 'Copy', 'Paste', 'PasteText', 'PasteFromWord', '-', 'Undo', 'Redo' ], { name: 'basicstyles', items: [ 'Bold', 'Italic', 'Underline' ] },
			{ name: 'links', items: [ 'Link', 'Unlink', 'Anchor' ] },
			{ name: 'insert', items: [ 'Image', 'Flash', 'Table', 'HorizontalRule', 'Smiley', 'SpecialChar', 'PageBreak', 'Iframe' ] },
 ...
 ?>
	...
	<?php foreach($parser["blocks"] as $_key_13=>$_var_13)
{
 ?>

	<?php if (''.$parser["blocks"][$_key_13]["TYPE"].''=='1') {?> 	
	<?php foreach($parser["langs"] as $_key_14=>$_var_14)
{
 ?>
	CKEDITOR.replace( 'text_<?php echo $parser["blocks"][$_key_13]["num"]; ?>_<?php echo $parser["langs"][$_key_14]["ID"]; ?>' , {
		toolbar: [ 
			{ name: 'document', items: [ 'Source', '-', 'NewPage' ] }, [ 'Cut', 'Copy', 'Paste', 'PasteText', 'PasteFromWord', '-', 'Undo', 'Redo' ], { name: 'basicstyles', items: [ 'Bold', 'Italic', 'Underline' ] },
			{ name: 'links', items: [ 'Link', 'Unlink', 'Anchor' ] },
			{ name: 'insert', items: [ 'Image', 'Flash', 'Table', 'HorizontalRule', 'Smiley', 'SpecialChar', 'PageBreak', 'Iframe' ] },
 ...
 ?>
	...
	DJS['addUpImages_<?php echo $parser["blocks"][$_key_13]["LEFT"]; ?>'] =  function(up, file) {
		var h = '<p class="itemimg data-type="" id="up_'+up+'_'+file.id+'" data-img=""><a class="del" href="javascript:delImageT(\''+up+'\', \''+file.id+'\', \'\');"></a><input type="hidden" name="itemimg['+up+'][]" value="" id="itemimg_'+up+'_'+file.id+'" /><img src="" style="display:none;" /><a class="upproc" style="margin-left: 11px; margin-top: 20px;">0%</a></p>';
		$('#addpbut_'+up).before(h);
		$( "#itemimg_" + up ).sortable({
			containment: "parent",
	...
	DJS['uploadUpImages_<?php echo $parser["blocks"][$_key_13]["LEFT"]; ?>'] =  function(up, file, resp, tm) {
		var obj = $.parseJSON(resp['response']);
		$('#up_'+tm+'_'+file.id+' a.upproc').css('display', 'none');
		$('#up_'+tm+'_'+file.id+' a.del').fadeIn();
		$('#up_'+tm+'_'+file.id+' img').load(function() {
	...
	DJS['progressUpImages_<?php echo $parser["blocks"][$_key_13]["LEFT"]; ?>'] =  function(up, file, tm) {
		$('#up_'+tm+'_'+file.id+' a.upproc').html(file.percent+'%');
	}


	addUploader(new Array(<?php echo $parser["blocks"][$_key_13]["LEFT"]; ?>, 'addimagebutt_<?php echo $parser["blocks"][$_key_13]["LEFT"]; ?>', 'upimgs_<?php echo $parser["blocks"][$_key_13]["LEFT"]; ?>', 'addUpImages_<?php echo $parser["blocks"][$_key_13]["LEFT"]; ?>', 'uploadUpImages_<?php echo $parser["blocks"][$_key_13]["LEFT"]; ?>', 'progressUpImages_<?php echo $parser["blocks"][$_key_13]["LEFT"]; ?>', '/<?php echo (cLANG); ?>/manager/<?php echo (OBJ); ?>/addimage/'));

	$( "#itemimg_<?php echo $parser["blocks"][$_key_13]["LEFT"]; ?>" ).sortable({
		containment: "parent",
		items: "p.itemimg",
		stop: function( event, ui ) {
			//$('#thpr').html('');
