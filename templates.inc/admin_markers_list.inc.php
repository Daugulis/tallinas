	...
	<div class="sort">
		...
		<div class="user-items-table">
        	<div class="table-header">
            	<div class="cell-1" style="width: 70%;"><?php echo (voc('Markers')); ?></div>
                <div class="cell-6" style="width: 20%;">ДЕЙСТВИЯ</div>
            </div>
			<div class="add-line">
            	<div class="cell-1">
					<a href="/<?php echo (cLANG); ?>/manager/markers/add/" class="add-category"><img src="/admin/img/icon-add-small.png" width="16" height="16" border="0" alt="" /><?php echo (voc('Add new')); ?></a>
                </div>
            </div>
...
function deleteMenu(id)
{
	if (confirm('Удалить запись?'))
	{
		$.ajax({
			type: 'POST',
			url: '/<?php echo (cLANG); ?>/manager/markers/deleteajax/'+id,
			data: ''
		}).done(function(data) {
...
{
	showRows('/<?php echo (cLANG); ?>/manager/markers/listajax/', 'utype='+$('#utype').val());
}
...
function editCompany(id)
{
	$.ajax({
		type: 'POST',
		url: '/<?php echo (cLANG); ?>/manager/markers/editajax/'+id,
		data: ''
	}).done(function(data) {
		$('#pcontent').html(data);
