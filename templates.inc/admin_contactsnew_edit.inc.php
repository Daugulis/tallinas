...
<div class="sub-menu">
	<div class="sub-menu-container">
    	<a href="/<?php echo (cLANG); ?>/manager/contacts/"><?php echo (voc('Contacts')); ?></a>
    </div>
</div>
<div class="main">
	<div class="sort">
	...
	<div class="add-table">
		<div class="table-header">
			<div class="cell-1" style="width: 100%;"><?php echo (voc('Edit contacts')); ?> - <?php echo (htmlspecialchars($parser['d_ADDRESS'])); ?></div>
		</div>
		<div class="add-item">
			<form id="pform">
					...
					<?php foreach($parser["langs"] as $_key_2=>$_var_2)
{
 ?>
					<div class="half-block">
						<h1><?php echo (voc('Address')); ?> (<?php echo $parser["langs"][$_key_2]["SHORTNAME"]; ?>):</h1>
						<input type="text" class="prev" name="address_<?php echo $parser["langs"][$_key_2]["ID"]; ?>" value="<?php echo (htmlspecialchars(getVFValue('d_address', ''.$parser["langs"][$_key_2]["ID"].''))); ?>" />
					</div>
					<?php 
}
 ?>	
					<div class="half-block">
						<h1><?php echo (voc('RegNR')); ?>:</h1>
						<input type="text" class="prev" name="regnr" value="<?php echo $parser["d_REGNR"]; ?>" />
					</div>
					<div class="half-block">
						<h1><?php echo (voc('E-mail')); ?>:</h1>
						<input type="text" class="prev" name="email" value="<?php echo $parser["d_EMAIL"]; ?>" />
					</div>
					<div class="half-block">
						<h1><?php echo (voc('Phone')); ?>:</h1>
						<input type="text" class="prev" name="phone1" value="<?php echo $parser["d_PHONE1"]; ?>" />
					</div>
					<div class="half-block">
						<h1><?php echo (voc('Phone')); ?>:</h1>
						<input type="text" class="prev" name="phone2" value="<?php echo $parser["d_PHONE2"]; ?>" />
					</div>
...
function saveText()
{
	...
	$.ajax({
		type: 'POST',
		url: '/<?php echo (cLANG); ?>/manager/contacts/editajax/<?php echo (ID); ?>/',
...
DJS.uploadUpImages =  function(up, file, resp) {
	...
	$('#up'+file.id+' img').attr('src', '/getimage.php?id=b'+obj.typeid+'1'+obj.id);
	...
	<?php foreach($parser["langs"] as $_key_4=>$_var_4)
{
 ...
 ?>
	...
	$("#mapinput").bind("geocode:dragged", function(event, latLng){          
		...
		$.getJSON("http://maps.googleapis.com/maps/api/geocode/json?latlng="+latLng.lat()+","+latLng.lng()+"&sensor=true", function(data) {
			var obj = data;
			$("#mapinput").val(obj.results[0].formatted_address);
			$("input[name=formatted_address]").val(obj.results[0].formatted_address); 
		});
