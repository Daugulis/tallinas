            ...
            $img.each(function () {
                ...
                $('<img />').load(function () {
					...
					if (wrz===true)
					{
						...
						var img2 = $('.m-p-img2 .imgc');
						img2.css('width', coverWidth + 'px');
						img2.css('height', coverHeight + 'px');
						img2.attr('width', coverWidth);
						img2.attr('height', coverHeight);
...
(function ($) {
    ...
    if (has_canvas) {
        ...
        add_shape_to = function (canvas, shape, coords, options, name, obb, obp) {
			...
			var optt = options_from_area(this, options);
            context.restore();
		...
		add_shape_to2 = function (canvas, shape, coords, options, name, obb, obp) {
			...
			var optt = options_from_area(this, options);
			console.log(obb);			
			var scc = getAreaCenter(shape, coords);
			console.log(scc);
			if ($(window).width()>600)
			{
				$('#plani').attr('center-x', scc[0]);
				$('#plani').attr('center-y', scc[1]);
				var cvv = scc[1] - 113;
				var cxx = scc[0];
				$('.plsl').css('margin-top', cvv+'px');
				$('.plsl').css('margin-left', cxx+'px');

				var grd = context.createLinearGradient(scc[0]-35,0,scc[0]+35,0);
				grd.addColorStop(0,"#0a83e0");
				grd.addColorStop(1,"#00b4ff");

				// Fill with gradient
				context.fillStyle = "rgba(255, 255, 255, 1)";
				context.fillRect(scc[0]-36, scc[1]-16, 72, 32);
				
				context.fillStyle = grd;
				context.fillRect(scc[0]-35, scc[1]-15, 70, 30);

				context.font = "12px Open Sans";
				context.textAlign = "center";
				context.fillStyle = "rgba(255, 255, 255, 1)";

				var nm = parseInt($(obb).attr('data-num'));
				nm = ( parseInt(floor) - 1 ) * nm;
				
				context.fillText(rooms[nm].area + 'm', scc[0], scc[1]+5);
	...
	*/
    ...
    $.fn.maphilight = function (opts) {
        ...
        return this.each(function () {
            var img, wrap, options, map, canvas, canvas_always, canvas_curr, mouseover, highlighted_shape, usemap;
            img = $(this);
            ...
            mouseover = function (e) {
				...
				!area_options.alwaysOn
                    ...
                    add_shape_to(canvas, shape[0], shape[1], area_options, "highlighted", this);
            ...
            $(map).bind('alwaysOn.maphilight', function () {
                ...
                $(map).find('area[coords]').each(function () {
                    ...
                    if (area_options.alwaysOn) {
                        ...
                        shape = shape_from_area(this);
                        if (has_canvas) {
                            add_shape_to(canvas_always, shape[0], shape[1], area_options, "", this, 1);
                        } else {
                            add_shape_to(canvas, shape[0], shape[1], area_options, "", this, 1);
                        }
                    }
                });
            });

			$(map).bind('curr.maphilight', function () {
                if (canvas_curr) {
                    clear_canvas(canvas_curr);
                }
                if (!has_canvas) {
                    $(canvas).empty();
                }
                $(map).find('area[data-curr="1"]').each(function () {
                    var shape, area_options;
                    area_options = options_from_area(this, options);
                    if (!canvas_curr && has_canvas) {
						canvas_curr = create_canvas_for(img[0]);
						$(canvas_curr).css(canvas_style);
						canvas_curr.width = img[0].width;
						canvas_curr.height = img[0].height;
						img.before(canvas_curr);
					}
					area_options.fade = area_options.alwaysOnFade; // alwaysOn shouldn't fade in initially
					shape = shape_from_area(this);
					if (has_canvas) {
						add_shape_to2(canvas_curr, shape[0], shape[1], area_options, "", this, 1);
					} else {
						add_shape_to2(canvas, shape[0], shape[1], area_options, "", this, 1);
					}
                });
            });
			...
			.bind('mouseout.maphilight', function (e) { $(this).attr('data-on', '0'); clear_canvas(canvas); if (canvas_curr)
			{
				clear_canvas(canvas_curr);
			} });
