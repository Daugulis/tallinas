...
class counters extends textsdata
{
	...
	{
		$sql = "delete from `tb_counters` where `ID` in (".$ids.")";
		$this->_parent->DB->Query($sql);
		$sql = "delete from `tb_counters_data` where `CID` in (".$ids.")";
		$this->_parent->DB->Query($sql);
	}
	...
	{
		$left = $this->getLeft() + 1;
		$data = Array("STATUS"=>$_POST['status'], "CN"=>$_POST['cn'], "MIN"=>$_POST['min'], "MAX"=>$_POST['max'], "LEFT"=>$left);
	...
	{
		$data = Array("STATUS"=>$_POST['status'], "CN"=>$_POST['cn'], "MIN"=>$_POST['min'], "MAX"=>$_POST['max']);
